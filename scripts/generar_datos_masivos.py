...
TIPOS_INC = ["qr_no_valido", "placa_no_coincide", "sin_autorizacion", "persona_sospechosa"]
...
if db.users.count_documents({"rol": "guardia"}) == 0:
    ...
    guardias_ref = [{"_id": _id, "nombre": n} for _id, n in zip(res.inserted_ids, guardias_meta)]
    print("Guardias creados")
else:
    # Ya existen: solo los cargamos para usarlos en logs/incidencias
    guardias_ref = [{"_id": g["_id"], "nombre": g["nombre"]}
...
for i in tqdm(range(TOTAL_RESIDENTES), desc="Residentes"):
    ...
    buf_meta.append({
        "nombre": nombre, "telefono": tel, "numero_casa": casa,
        "privada": privada, "fraccionamiento": "cedros prueba",
    })
...
for i in tqdm(range(TOTAL_VISITAS), desc="Visitas"):
    ...
    buf_docs.append({
        ...
        "fecha_visita": fake.date_between(start_date="-180d", end_date="+30d").strftime("%Y-%m-%d"),
    ...
    buf_meta.append({"nombre_visitante": nombre_vis, "residencia_destino": residente["numero_casa"]})
